            ...
            <div class="row">
                <div class="col-md-12"> <span class="heading-meta style-1">Say About Me</span>
                    ...
                    @foreach($cats as $category)
                        <a class="btn btn-primary" href="{{route('podcast.index', ['category_id'=>$category->id])}}">{{ $category->name }}</a>
                    @endforeach
                ...
                @foreach ($podcasts as $podcast)
                    <div class="col-md-6 animate-box" data-animate-effect="fadeInLeft">
                        <div class="techynaf-quote-card"> <img src="frontend/images/quote.png" alt="" class="techynaf-quote-2">
                            ...
                            <h5>{{ $podcast->title }}</h5>
                            <a target="_blank" href="{{ $podcast->url }}" class="occupation">Visit</a>
                            <a href="{{route('podcast.edit', ['podcast' => $podcast->id])}}" class="occupation">Edit</a>
                        </div>
                    </div>
                @endforeach

                <a href="{{route('podcast.index')}}" class="btn btn-primary">See More</a>
            </div>
        </div>
    </div>
@endguest
